    ...
    implements TrueZip
{
    ...
    {
        new DefaultTrueZipArchiveDetector();
    }
        ...
        throws IOException
    {
        ...
        {
            // TFile#isDirectory() is archive-suffix-aware: a real file on disk whose
            // suffix isn't registered with an archive driver is NOT treated as a
            // virtual directory, and reports false here even though the file exists.
            // Disambiguate that case from a genuinely missing path so the error
            // message points at the actual problem.
            File plainFile = new File( oneFileSet.getDirectory() );

            if ( !plainFile.exists() )
            {
                throw new IOException( "FileSet's directory: " + directory + " not found." );
            }
            else if ( plainFile.isFile() && !directory.isArchive() )
            {
                throw new IOException( "FileSet's directory: " + directory + " exists as a plain file, "
                    + "but its suffix is not registered with an archive driver, so TrueZIP cannot browse "
                    + "into it as a virtual directory. If this is meant to be an archive, check that its "
                    + "file suffix (e.g. \"" + getFileExtension( directory.getPath() )
                    + "\") is one of the suffixes recognized by the configured archive detector." );
            }
            else
            {
                throw new IOException( "FileSet's directory: " + directory + " is not a directory." );
            }
        }
