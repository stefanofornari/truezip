    ...
    extends TestCase
{
    ...
    public void testNbmIsJarArchive()
        throws Exception
    {
        DefaultTrueZipArchiveDetector trueZipArchiveDetector = new DefaultTrueZipArchiveDetector();
        trueZipArchiveDetector.init();

        // Build a plain zip-shaped fixture with a .nbm suffix using the JDK's own
        // zip support - deliberately not TrueZIP - so this test doesn't depend on
        // .nbm already being a recognized suffix just to create the fixture file.
        File nbmFile = new File( basedir.getAbsolutePath(), "target/test-nbm/sample.nbm" );
        nbmFile.getParentFile().mkdirs();
        if ( nbmFile.exists() )
        {
            nbmFile.delete();
        }
        ...
        java.util.zip.ZipOutputStream zos =
            new java.util.zip.ZipOutputStream( new java.io.FileOutputStream( nbmFile ) );
        try
        {
            zos.putNextEntry( new java.util.zip.ZipEntry( "netbeans/modules/ext/some-native.jar" ) );
            zos.write( "not-a-real-jar".getBytes() );
            zos.closeEntry();

            zos.putNextEntry( new java.util.zip.ZipEntry( "netbeans/config/Modules/some.xml" ) );
            zos.write( "<module/>".getBytes() );
            zos.closeEntry();
        }
        finally
        {
        ...
        TFile file = new TFile( basedir, "target/test-nbm/sample.nbm" );
        assertTrue( "fixture .nbm file should exist on disk", file.exists() );
        assertTrue( ".nbm should be recognized as an archive, not a plain file", file.isArchive() );
        assertTrue( ".nbm should be browsable as a virtual directory", file.isDirectory() );
        ...
        assertEquals( "Invalid file list in " + file, 2, files.size() );
    }
